...
const normalizeMySQLError = require('./normalize-error');
...
MySQL.prototype.executeSQL = function(sql, params, options, callback) {
  ...
  function handleResponse(connection, err, result) {
    ...
    if (err) {
      if (debugEnabled) {
        debug('Raw driver error intercepted: %O', err);
      }
      // Transform raw MySQL driver error into standard LoopBack DB error
      err = normalizeMySQLError(err);
    }
