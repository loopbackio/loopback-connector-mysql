...
MySQL.prototype.toColumnValue = function(prop, val) {
  ...
  if (prop.type === Date) {
    ...
    if (prop.mysql && prop.mysql.dataType == 'date') {
        //take only date part
        val = dateToMysql(val).substr(0,10);
    } else if (prop.mysql && prop.mysql.dataType == 'datetime') {
        //convert to string
        val = dateToMysql(val);
    }
    //pass value to mySql as is
    return val;
  }
  ...
  if (prop) {
    switch (prop.type.name) {
      ...
      case 'Date':
        ...
        // those separate.
        //if options.dateStrings == true format date else keep it as is
        if (val.toString().match(/\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d/)) {
