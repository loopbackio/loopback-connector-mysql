...
MySQL.prototype.toFields = function (model, data) {
  ...
  Object.keys(data).forEach(function (key) {
    if (props[key]) {
      ...
      // skip property if it cannot be NULL, but `toDatabase` function returns NULL
      var hasNullableOption = props[key].hasOwnProperty('mysql') &&
          props[key].mysql.hasOwnProperty('nullable');

      if (hasNullableOption && value === 'NULL') {
        var nullable = props[key].mysql.nullable;
        if (nullable === 'N' || nullable === 'NO' || nullable === false) {
          return;
        }
      }
