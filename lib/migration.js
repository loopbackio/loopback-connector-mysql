...
function mixinMigration(MySQL, mysql) {
  ...
  MySQL.prototype.addNewForeignKeys = function(model, actualIndexes) {
    //add new foreign keys
    var self = this;
  ...
  MySQL.prototype.getColumnsToAdd = function(model, actualFields) {
    ...
    propNames.forEach(function(propName) {
      ...
      var found;
      var colName = self.expectedColNameForModel(propName, m);
      if (actualFields) {
  ...
  MySQL.prototype.expectedColNameForModel = function(propName, modelToCheck) {
  ...
  MySQL.prototype.dropForeignKeysForRemoveFields = function(model, actualFks, sqlStmt) {
    var self = this;
    var m = self._models[model];
    ...
    if (fks) {
      ...
      fks.forEach(function(fk) {
        ...
        if (newFk) {
          var fkCol = self.expectedColNameForModel(newFk.foreignKey, m);
          var fkEntity = self.getModelDefinition(newFk.entity);
          var fkRefKey = self.expectedColNameForModel(newFk.entityKey, fkEntity);
  ...
  MySQL.prototype.alterTable = function(model, actualFields, actualIndexes, actualFks, done, checkOnly) {
    ...
    var aiNames = this.addNewForeignKeys(model, actualIndexes)[0];
    var ai = this.addNewForeignKeys(model, actualIndexes)[1];

    sql = this.getAddModifyColumns(model, actualFields);

    actualFks = this.dropForeignKeysForRemoveFields(model, actualFks, sql)[0];
    sql = this.dropForeignKeysForRemoveFields(model, actualFks, sql)[1];

    // drop columns
    if (actualFields) {
      actualFields.forEach(function(f) {
        var colNames = propNames.map(function expectedColName(propName) {
          return self.expectedColNameForModel(propName, m);
        });
