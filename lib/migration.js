...
function mixinMigration(MySQL, mysql) {
  ...
  MySQL.prototype.dropForeignKeysForRemoveFields = function(model, actualFks) {
    var self = this;
    var m = self._models[model];
  ...
  MySQL.prototype.addIndexes = function(model, actualIndexes, cb) {
    var self = this;
    var m = self._models[model];
    ...
    if (sql.length > 0) {
      self.applySqlChanges(model, sql, function(err, result) {
        if (err) cb(err);
        else
          cb(err, result);
      });
  ...
  MySQL.prototype.alterTable = function(model, actualFields, actualIndexes, actualFks, done, checkOnly) {
    ...
    actualFks = this.dropForeignKeysForRemoveFields(model, actualFks)[0];
    sql = sql.concat(this.dropForeignKeysForRemoveFields(model, actualFks)[1]);
    ...
    async.series([
      function(cb) {
        applyPending([self.getAddModifyColumns], cb);
      },
      function(cb) {
        self.addIndexes(model, actualIndexes, cb);
      },
      function(cb) {
        applyPending([self.getDropColumns], cb);
      },
      // function(cb) {
      //   self.executeAlterStatement(model, actualFks, sql, cb);
      // },
    ], function(err, result) {
      done(err, result[0]);
    });
  ...
  MySQL.prototype.applySqlChanges = function(model, pendingChanges, cb) {
    var self = this;
  ...
  MySQL.prototype.executeAlterStatement = function(model, actualFks, sqlStmt, cb) {
    var self = this;
    var m = self._models[model];
    var sql = stmt;
