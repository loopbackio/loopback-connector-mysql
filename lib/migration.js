...
function mixinMigration(MySQL, mysql) {
   ...
   */
  MySQL.prototype.getTableStatus = function(model, cb) {
    var fields;
    var indexes;
    ...
    function tableFieldCallback(err, data) {
      if (err) {
        console.error(err);
      }
      fields = data;
    ...
    function tableIndexCallback(err, data) {
      if (err) {
        console.log(err);
      }
      indexes = data;
    ...
    this.execute('SHOW FIELDS FROM ' + table, tableFieldCallback);
    this.execute('SHOW INDEXES FROM ' + table, tableIndexCallback);
  };
  ...
  MySQL.prototype.isActual = function(models, cb) {
    ...
    async.each(models, function(model, done) {
      self.getTableStatus(model, function(err, fields, indexes, foreignKeys) {
